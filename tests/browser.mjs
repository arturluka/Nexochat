...
window.__pcs=[];const Native=window.RTCPeerConnection;window.RTCPeerConnection=class extends Native{constructor(...args){super(...args);window.__pcs.push(this);}};});
 ...
 await alice.getByRole('button',{name:'Iniciar chamada'}).click();await mkdir('docs/screenshots',{recursive:true});await alice.screenshot({path:'docs/screenshots/call-lobby.png',fullPage:true});await alice.getByRole('button',{name:/^Conversar/}).click();await alice.getByText('Conectado · áudio em tempo real').waitFor();await bob.getByRole('dialog',{name:'Chamada recebida'}).waitFor();assert.equal(await bob.evaluate(()=>window.__audioRequests),0);await bob.getByRole('button',{name:'Aceitar',exact:true}).click();await bob.getByText('Conectado · áudio em tempo real').waitFor();
