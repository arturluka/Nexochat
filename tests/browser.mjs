...
window.__pcs=[];const Native=window.RTCPeerConnection;window.RTCPeerConnection=class extends Native{constructor(...args){super(...args);window.__pcs.push(this);}};});
 ...
 await alice.getByRole('button',{name:'Iniciar chamada'}).click();await mkdir('docs/screenshots',{recursive:true});await alice.screenshot({path:'docs/screenshots/call-lobby.png',fullPage:true});await alice.getByRole('button',{name:'Testar microfone antes de entrar'}).click();await alice.getByRole('button',{name:'Parar teste'}).waitFor();await alice.getByRole('button',{name:'Parar teste'}).click();assert.equal(await alice.evaluate(()=>window.__captures.every(s=>s.getTracks().every(t=>t.readyState==='ended'))),true);await alice.getByRole('button',{name:/^Conversar/}).click();await alice.getByText('Conectado · áudio em tempo real').waitFor();await bob.getByRole('dialog',{name:'Chamada recebida'}).waitFor();assert.equal(await bob.evaluate(()=>window.__audioRequests),0);await bob.getByRole('button',{name:'Aceitar',exact:true}).click();await bob.getByText('Conectado · áudio em tempo real').waitFor();
 ...
 await alice.getByTitle('Compartilhar tela',{exact:true}).click();await bob.locator('.screen-tile').waitFor();await bob.getByTitle('Destacar transmissão').click();await bob.locator('.featured-video').waitFor();await bob.getByTitle('Tirar destaque').click();
 ...
 await alice.getByRole('button',{name:'Dispositivos',exact:true}).click();await alice.getByLabel('Ouvir meu outro dispositivo (teste com fones)').check();assert.equal(await alice.locator('.same-account audio').first().evaluate(el=>el.muted),false);await alice.getByLabel('Ouvir meu outro dispositivo (teste com fones)').uncheck();
 ...
 await alice.setViewportSize({width:1440,height:960});await alice.getByRole('button',{name:/Loja de molduras/}).first().click();await alice.getByRole('button',{name:'Resgatar 100 Faíscas'}).click();await alice.getByText('Você ganhou 100 Faíscas!',{exact:true}).waitFor();
 await alice.locator('.frame-product').filter({has:alice.getByRole('heading',{name:'Órbita',exact:true})}).getByRole('button',{name:'Adquirir'}).click();await alice.getByRole('button',{name:'Confirmar com Faíscas'}).click();await alice.getByRole('button',{name:'Equipar',exact:true}).click();await alice.getByText('Moldura equipada!',{exact:true}).waitFor();
 assert.equal((await (await a.request.get(origin+'/api/state')).json()).me.equipped_frame,'orbit');await alice.screenshot({path:'docs/screenshots/shop-desktop.png',fullPage:true});await alice.setViewportSize({width:390,height:844});await alice.screenshot({path:'docs/screenshots/shop-mobile.png',fullPage:true});assert.equal(await alice.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false);
 console.log('PASS: chamada recebida/aceita, áudio do outro dispositivo, resgate, compra e moldura equipada.');
 await alice.setViewportSize({width:1440,height:960});await alice.getByRole('button',{name:'Insígnias e conquistas',exact:true}).click();
 await alice.locator('.achievement').filter({has:alice.getByRole('heading',{name:'Primeiro Nexo',exact:true})}).getByRole('button',{name:'Resgatar conquista'}).click();
 await alice.getByText('Primeiro Nexo conquistada! +25 Faíscas',{exact:true}).waitFor();await alice.getByRole('button',{name:'Exibir no perfil',exact:true}).click();await alice.getByText('Vitrine do perfil atualizada!',{exact:true}).waitFor();
 assert.deepEqual((await (await a.request.get(origin+'/api/state')).json()).me.displayed_badges,['arrival']);await alice.getByLabel('Insígnias do perfil').getByLabel('Primeiro Nexo',{exact:true}).waitFor();
 await alice.screenshot({path:'docs/screenshots/badges-desktop.png',fullPage:true});await alice.setViewportSize({width:390,height:844});await alice.screenshot({path:'docs/screenshots/badges-mobile.png',fullPage:true});assert.equal(await alice.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false);
 console.log('PASS: resgatar conquista, equipar insígnia, vitrine pública e layout responsivo.');
