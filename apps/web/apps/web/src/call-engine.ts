...
export type RemotePeer={id:string;name:string;user_id:string;pc:RTCPeerConnection;camera:MediaStream;screen:MediaStream;audio:MediaStream;screenAudio:MediaStream;media:MediaState;connection:string;initiator:boolean;retries:number;rttMs:number|null;lossPercent:number|null;packetsReceived:number;packetsLost:number;audioReceived:number;audioSent:number;audioReceiving:boolean;audioSending:boolean;route:string;pending:RTCIceCandidateInit[];queue:Promise<void>};
...
export class CallEngine {
 ...
 private createPeer(id:string,name:string,user_id:string,media?:MediaState){
  ...
  const pc=new RTCPeerConnection(this.config);const p:RemotePeer={id,name,user_id,pc,camera:new MediaStream(),screen:new MediaStream(),audio:new MediaStream(),screenAudio:new MediaStream(),media:media||emptyMedia(),connection:'connecting',initiator:false,retries:0,rttMs:null,lossPercent:null,packetsReceived:0,packetsLost:0,audioReceived:0,audioSent:0,audioReceiving:false,audioSending:false,route:'Ainda negociando',pending:[],queue:Promise.resolve()};this.peers.set(id,p);
 ...
 async stats(){for(const p of this.peers.values()){if(p.pc.signalingState==='closed')continue;try{let rx=0,tx=0,packets=0,lost=0;const report=await p.pc.getStats();for(const stat of report.values()){if(stat.kind==='audio'&&stat.type==='inbound-rtp'){rx+=stat.bytesReceived||0;packets+=stat.packetsReceived||0;lost+=stat.packetsLost||0;}if(stat.kind==='audio'&&stat.type==='outbound-rtp')tx+=stat.bytesSent||0;}const got=Math.max(0,packets-p.packetsReceived),missing=Math.max(0,lost-p.packetsLost);p.lossPercent=got+missing?missing/(got+missing)*100:null;p.packetsReceived=packets;p.packetsLost=lost;p.audioReceiving=rx>p.audioReceived;p.audioSending=tx>p.audioSent;p.audioReceived=rx;p.audioSent=tx;const transport=[...report.values()].find((s:any)=>s.type==='transport'&&s.selectedCandidatePairId);const pair=transport?report.get(transport.selectedCandidatePairId):[...report.values()].find((s:any)=>s.type==='candidate-pair'&&s.nominated&&s.state==='succeeded');const local=pair&&report.get(pair.localCandidateId),remote=pair&&report.get(pair.remoteCandidateId);p.rttMs=typeof pair?.currentRoundTripTime==='number'?pair.currentRoundTripTime*1000:null;p.route=pair?(local?.candidateType==='relay'||remote?.candidateType==='relay'?'Via TURN':'Direta'):'Sem rota de mídia';}catch{}}this.update();}
