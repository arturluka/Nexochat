...
import {communityPlus} from './community-plus.js';
import {eventExtras} from './event-extras.js';
import {rtcSettings} from './rtc-config.js';
import {securityService} from './security.js';
import {everydayFeatures} from './everyday.js';
import {communityFeatures,guardSend} from './community-features.js';
...
const publicColumns = 'id,username,display_name,bio,avatar_id,status,banner_id,accent_color,pronouns,custom_status,equipped_frame,displayed_badges,name_color,banner_style,equipped_background,equipped_color,equipped_effect';
...
export function createApp(db:DB) {
 ...
 app.use('/api',rateLimit({windowMs:60_000,limit:1200,standardHeaders:'draft-8',legacyHeaders:false,message:{error:'Muitas requisições. Aguarde um minuto.'}}));
 ...
 async function serverAccess(user:string,server:string,permission?:string) {
   const row=await first('SELECT s.*,m.role_id,m.timeout_until,m.accepted_rules,r.permissions FROM servers s JOIN server_members m ON m.server_id=s.id LEFT JOIN roles r ON r.id=m.role_id WHERE s.id=$1 AND m.user_id=$2 AND NOT m.banned',[id.parse(server),user]);
   if(!row) fail(403,'Sem acesso à comunidade');
   if(['send','call'].includes(permission||'')&&row.owner_id!==user&&row.rules&&row.accepted_rules<row.rules_version)fail(403,'Leia e aceite as regras em Boas-vindas e cargos antes de participar');
   if(['send','call'].includes(permission||'')&&row.timeout_until&&new Date(row.timeout_until).getTime()>Date.now())fail(403,'Você está em timeout até '+new Date(row.timeout_until).toISOString());
   if(permission && permission!=='call' && row.owner_id!==user && !(row.permissions||['send','invite']).includes(permission)) fail(403,'Seu cargo não permite esta ação');
   return row;
 }
 async function access(user:string,roomId:string,permission?:string) {
   ...
   if(room.server_id){const s=await serverAccess(user,room.server_id,permission);if(room.private&&s.owner_id!==user&&!await first('SELECT 1 FROM room_roles WHERE room_id=$1 AND role_id=$2',[room.id,s.role_id]))fail(403,'Canal privado: seu cargo não tem acesso');}
   else {
 ...
 async function members(room:any) {const rows=await db.query(room.server_id?'SELECT user_id FROM server_members WHERE server_id=$1 AND NOT banned':'SELECT user_id FROM room_members WHERE room_id=$1',[room.server_id||room.id]);if(!room.private)return rows;const allowed=[];for(const m of rows)try{await access(m.user_id,room.id);allowed.push(m);}catch(e:any){if(![403,404].includes(e.status))throw e;}return allowed;}
 async function notify(user:string,actor:string,body:string,room:string|null=null,mention=true) { if(user===actor || await blocked(user,actor)) return;if(room){let r;try{r=await access(user,room);}catch(e:any){if([403,404].includes(e.status))return;throw e;}const p=await first('SELECT muted,notification_mode FROM room_members WHERE room_id=$1 AND user_id=$2',[room,user]);const mode=p?.notification_mode||(p?.muted?'muted':r.server_id?'mentions':'all');if(mode==='muted'||mode==='mentions'&&!mention)return;} await db.query('INSERT INTO notifications(id,user_id,actor_id,body,room_id) VALUES($1,$2,$3,$4,$5)',[uid(),user,actor,body,room]); }
 ...
 const security=securityService(db);
 const disconnectUser=(user:string)=>{for(const s of io.sockets.sockets.values())if(s.data.user.id===user)s.disconnect(true);};
 security.publicRoutes({app,route,loginLimit,disconnectUser});
 ...
 app.get('/api/health',(_req,res)=>res.json({ok:true,version:'0.8.0',database:process.env.DATABASE_URL?'postgresql':'pglite'}));
 app.use('/api',auth);
 ...
 app.use('/api/security',rateLimit({windowMs:60_000,limit:10,keyGenerator:req=>(req as AuthRequest).user.id,skip:req=>req.method==='GET',message:{error:'Muitas alterações de segurança. Aguarde um minuto.'}}));
 security.privateRoutes({app,route});
 ...
 app.post('/api/shop/equip',commerceLimit,route(async(req,res)=>{const x=z.object({item_id:z.string().max(30).nullable(),kind:z.enum(['frame','background','color','effect']).default('frame')}).strict().parse(req.body);res.json(await shop.equip(req.user.id,x.item_id,x.kind));refresh();}));
 app.post('/api/shop/gift',commerceLimit,route(async(req,res)=>{const x=z.object({username,item_id:z.string().max(30)}).strict().parse(req.body);res.json(await shop.gift(req.user.id,x.username,x.item_id));refresh();}));
 ...
 app.get('/api/state',route(async(req,res)=>{
   const me=req.user;
   const rooms=await db.query(`SELECT DISTINCT r.*,rm.muted,rm.notification_mode,rm.last_read_at,
     ...
     WHERE (rm.user_id IS NOT NULL AND r.server_id IS NULL OR sm.user_id IS NOT NULL AND NOT sm.banned) AND (r.expires_at IS NULL OR r.expires_at>now()) AND (NOT r.private OR EXISTS(SELECT 1 FROM servers ps WHERE ps.id=r.server_id AND ps.owner_id=$1) OR EXISTS(SELECT 1 FROM room_roles pr WHERE pr.room_id=r.id AND pr.role_id=sm.role_id)) ORDER BY r.created_at`,[me.id]);
   const servers=await db.query('SELECT s.*,m.role_id,m.accepted_rules,COALESCE(r.permissions,\'["send","invite"]\'::jsonb) AS permissions FROM servers s JOIN server_members m ON m.server_id=s.id LEFT JOIN roles r ON r.id=m.role_id WHERE m.user_id=$1 AND NOT m.banned ORDER BY s.created_at',[me.id]);
   ...
   for(let i=notifications.length-1;i>=0;i--)if(notifications[i].room_id)try{await access(me.id,notifications[i].room_id);}catch(e:any){if([403,404].includes(e.status))notifications.splice(i,1);else throw e;}
   res.json({me:{id:me.id,username:me.username,display_name:me.display_name,bio:me.bio,avatar_id:me.avatar_id,status:me.status,dm_policy:me.dm_policy,read_receipts:me.read_receipts,banner_id:me.banner_id,accent_color:me.accent_color,pronouns:me.pronouns,custom_status:me.custom_status,equipped_frame:me.equipped_frame,sparks:me.sparks,displayed_badges:me.displayed_badges,theme:me.theme,name_color:me.name_color,banner_style:me.banner_style,accessibility:me.accessibility,equipped_background:me.equipped_background,equipped_color:me.equipped_color,equipped_effect:me.equipped_effect},rooms,servers,relations,users,categories,notifications,community_profiles:await db.query('SELECT m.server_id,m.user_id,m.nickname,m.bio,m.avatar_id FROM server_members m WHERE NOT m.banned AND EXISTS(SELECT 1 FROM server_members self WHERE self.server_id=m.server_id AND self.user_id=$1 AND NOT self.banned)',[me.id])});
 }));
 ...
 app.post('/api/rooms',route(async(req,res)=>{
   ...
   if(x.server_id){ if(!['text','voice','temporary','forum'].includes(x.kind))fail(400,'Tipo de canal inválido');await serverAccess(req.user.id,x.server_id,'manage_channels');if(x.category_id&&!await first('SELECT 1 FROM categories WHERE id=$1 AND server_id=$2',[x.category_id,x.server_id]))fail(400,'Categoria inválida'); }
 ...
 app.delete('/api/servers/:id',route(async(req,res)=>{
  const sid=id.parse(req.params.id);const result=await db.transaction(async tx=>{const [s]=await tx.query('SELECT * FROM servers WHERE id=$1 FOR UPDATE',[sid]);if(!s)fail(404,'Comunidade não encontrada');if(s.owner_id!==req.user.id)fail(403,'Somente o dono pode excluir a comunidade');const name=z.string().max(60).parse(req.body?.name);if(name!==s.name)fail(400,'Digite o nome exato da comunidade para confirmar');const rooms=await tx.query('SELECT id FROM rooms WHERE server_id=$1',[sid]);const files=await tx.query('SELECT a.id FROM attachments a JOIN rooms r ON r.id=a.room_id WHERE r.server_id=$1',[sid]);await tx.query('DELETE FROM servers WHERE id=$1',[sid]);return {rooms,files};});
  for(const r of result.rooms)await evictRoom(r.id);for(const f of result.files)try{await unlink(path.join(dataDir,'uploads',f.id));}catch(e:any){if(e.code!=='ENOENT')console.error('Falha ao remover anexo da comunidade excluída',e.code);}refresh();res.json({ok:true});
 }));
 ...
 personalSpace({app,db,route,serverAccess});
 communityFeatures({app,db,route,access,serverAccess,blocked,refresh,evictRoom});
 communityPlus({app,db,route,access,serverAccess,blocked,refresh,evictRoom,members,notify});
 const events=eventExtras({app,db,route,serverAccess,refresh});
 const everyday=everydayFeatures({app,db,route,access,serverAccess,blocked,refresh,notify,members});
 app.post('/api/rooms/:id/messages',sendLimit,route(async(req,res)=>{const room=await access(req.user.id,String(req.params.id),'send');if(room.kind==='forum')fail(400,'Crie um assunto ou responda dentro de um tópico do fórum');const x=messageInput.parse(req.body);if(x.reply_id&&!await first('SELECT 1 FROM messages WHERE id=$1 AND room_id=$2',[x.reply_id,room.id]))fail(400,'Resposta inválida');if(x.attachment_id&&!await first('SELECT 1 FROM attachments WHERE id=$1 AND owner_id=$2 AND room_id=$3',[x.attachment_id,req.user.id,room.id]))fail(400,'Anexo inválido');if(x.sticker_id&&!await first("SELECT 1 FROM community_assets WHERE id=$1 AND server_id=$2 AND kind='sticker'",[x.sticker_id,room.server_id]))fail(400,'Figurinha indisponível nesta comunidade');const mid=uid();const perm=room.server_id?await serverAccess(req.user.id,room.server_id):null;await db.transaction(async tx=>{await guardSend(tx,req.user.id,room,x.body,room.server_id?(perm?.owner_id===req.user.id||(perm?.permissions||[]).includes('manage_messages')):room.owner_id===req.user.id);await tx.query('INSERT INTO messages(id,room_id,user_id,body,reply_id,attachment_id,sticker_id) VALUES($1,$2,$3,$4,$5,$6,$7)',[mid,room.id,req.user.id,x.body,x.reply_id||null,x.attachment_id||null,x.sticker_id||null]);});for(const m of await members(room)){const u=await first('SELECT username FROM users WHERE id=$1',[m.user_id]);const mention=new RegExp('(^|\\s)@'+u.username+'(?=\\s|$|[.,!?])').test(x.body);const directReply=x.reply_id&&await first('SELECT 1 FROM messages WHERE id=$1 AND user_id=$2',[x.reply_id,m.user_id]);await notify(m.user_id,req.user.id,`${req.user.display_name}: ${x.body.slice(0,100)||'Enviou um anexo'}`,room.id,mention||!!directReply);}refresh();res.json({id:mid});}));
 ...
 app.post('/api/upload',rateLimit({windowMs:3600_000,limit:30,keyGenerator:req=>(req as AuthRequest).user.id}),upload.single('file'),route(async(req,res)=>{
   ...
   if(req.query.voice==='true'){if(!room||!['video/webm','video/mp4'].includes(mime))fail(400,'Gravação inválida: use WebM ou MP4.');mime=mime.replace('video/','audio/');}
 ...
 app.get('/api/files/:id',route(async(req,res)=>{const a=await first('SELECT * FROM attachments WHERE id=$1',[id.parse(req.params.id)]);if(!a)fail(404,'Arquivo não encontrado');if(a.room_id){await access(req.user.id,a.room_id);if(await blocked(req.user.id,a.owner_id))fail(403,'Arquivo indisponível');if(a.owner_id!==req.user.id&&!await first('SELECT 1 FROM messages WHERE attachment_id=$1 AND NOT deleted',[a.id]))fail(403,'Arquivo não publicado');}else if(a.owner_id!==req.user.id&&!await first('SELECT 1 FROM users WHERE avatar_id=$1 OR banner_id=$1',[a.id])){const asset=await first('SELECT server_id FROM community_assets WHERE attachment_id=$1 UNION SELECT server_id FROM server_members WHERE avatar_id=$1 AND NOT banned UNION SELECT id AS server_id FROM servers WHERE icon_id=$1 OR banner_id=$1',[a.id]);if(!asset)fail(403,'Arquivo privado');await serverAccess(req.user.id,asset.server_id);}res.setHeader('Cache-Control','private, no-store');res.type(a.mime);if(!a.mime.startsWith('image/')&&!a.mime.startsWith('audio/'))res.attachment(a.name);res.send(await readFile(path.join(dataDir,'uploads',a.id)));}));
 app.get('/api/rtc-config',route(async(_req,res)=>{res.setHeader('Cache-Control','private, no-store');res.json(rtcSettings());}));
 ...
 app.use(express.static(path.join(root,'apps/web/dist'),{setHeaders:(res,file)=>{if(file.endsWith('.html')||['version.json','sw.js'].includes(path.basename(file)))res.setHeader('Cache-Control','no-store');}}));
 app.get('/{*path}',(req,res,next)=>{if(req.path.startsWith('/api/'))return res.status(404).json({error:'Rota não encontrada'});res.setHeader('Cache-Control','no-store');res.sendFile(path.join(root,'apps/web/dist/index.html'),e=>{if(e)next(e);});});
 ...
 return {app,http,io,tickScheduled:everyday.tick,tickReminders:events.tick,close:async()=>{clearInterval(sweep);clearInterval(ringSweep);await everyday.close();await events.close();await new Promise<void>(resolve=>io.close(()=>resolve()));await db.close();}};
}
