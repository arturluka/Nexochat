...
// This MVP runs one API instance; in-memory invitations cannot survive a restart.
await db.query("UPDATE call_history SET status='cancelled',answered_at=now() WHERE status='ringing'");
