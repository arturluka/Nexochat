...
## Versão 0.8.0 — espaço pessoal e atualizações

Build TypeScript/Vite concluído. Os cinco testes da API passaram, incluindo migração 008, segurança, permissões, denúncias, busca, chamadas e restauração de backups. O teste de moderação foi atualizado para exigir uma decisão registrada.

O teste personal-space-browser.mjs passou no Edge: pastas com persistência e isolamento entre contas, ordem das comunidades, layout de 390 px sem overflow, busca por tipo/nome de anexo sem acesso a conversas alheias, decisões de denúncia protegidas por permissão e acompanhadas pelo autor, gravação local e reprodução do teste de voz, identificação de versão e detecção de uma atualização simulada. A primeira execução precisou de maior espera pela inicialização do banco; um seletor de formulário do teste foi corrigido antes da execução final.

O teste browser.mjs também passou: perfis, loja, insígnias, áudio/vídeo WebRTC entre navegadores, câmera e tela simultâneas, cancelamento de permissões, modo só ouvir, microfone ausente, ativação tardia, PC + celular na mesma conta e limpeza de mídia. As capturas de pastas mobile e novidades foram inspecionadas.

Limites: mídia e dispositivos simulados no navegador, sem validação em celular físico, Safari/iOS ou TURN externo. Não houve publicação no GitHub/Railway nesta rodada. O aviso de atualização depende de uma publicação nova bem-sucedida; não corrige um deploy que continua usando código antigo.
