...
const publicColumns = 'id,username,display_name,bio,avatar_id,status';
...
export function createApp(db:DB) {
 ...
 app.get('/api/state',route(async(req,res)=>{
   ...
   res.json({me:{id:me.id,username:me.username,display_name:me.display_name,bio:me.bio,avatar_id:me.avatar_id,status:me.status,dm_policy:me.dm_policy,read_receipts:me.read_receipts},rooms,servers,relations,users,categories,notifications});
 }));
 app.patch('/api/profile',route(async(req,res)=>{const x=profile.parse(req.body);if(x.avatar_id&&!await first('SELECT 1 FROM attachments WHERE id=$1 AND owner_id=$2 AND room_id IS NULL AND mime LIKE \'image/%\'',[x.avatar_id,req.user.id])) fail(400,'Avatar inválido');await db.query('UPDATE users SET display_name=$2,bio=$3,status=$4,dm_policy=$5,read_receipts=$6,avatar_id=$7 WHERE id=$1',[req.user.id,x.display_name,x.bio,x.status,x.dm_policy,x.read_receipts,x.avatar_id??null]);refresh();res.json({ok:true});}));
 ...
 async function leaveCall(s:any){const room=s.data.call;if(room){s.to('call:'+room).emit('call-left',{peer:s.id});await s.leave('call:'+room);s.data.call=null;s.emit('call-ended');}}
 ...
 io.on('connection',s=>{
   ...
   const event=(name:string,handler:(p:any)=>Promise<any>)=>s.on(name,async(p,ack)=>{try{const result=await handler(p);if(typeof ack==='function')ack({ok:true,...result});}catch(e:any){if(typeof ack==='function')ack({ok:false,error:e.message});}});
