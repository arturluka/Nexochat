...
const publicColumns = 'id,username,display_name,bio,avatar_id,status,banner_id,accent_color,pronouns,custom_status,equipped_frame,displayed_badges,name_color,banner_style';
...
export function createApp(db:DB) {
 ...
 async function serverAccess(user:string,server:string,permission?:string) {
   const row=await first('SELECT s.*,m.role_id,m.timeout_until,r.permissions FROM servers s JOIN server_members m ON m.server_id=s.id LEFT JOIN roles r ON r.id=m.role_id WHERE s.id=$1 AND m.user_id=$2 AND NOT m.banned',[id.parse(server),user]);
 ...
 app.get('/api/health',(_req,res)=>res.json({ok:true,version:'0.6.2',database:process.env.DATABASE_URL?'postgresql':'pglite'}));
 app.use('/api',auth);
 ...
 return {app,http,io,tickScheduled:everyday.tick,close:async()=>{clearInterval(sweep);clearInterval(ringSweep);await everyday.close();await new Promise<void>(resolve=>io.close(()=>resolve()));await db.close();}};
}
